/*
 Arduino Library for Maxim Integrated's DS1339 RTC
...
DS1339::DS1339() : _rtc_int_pin(2), _rtc_int_number(0)
{
  init();
}

DS1339::DS1339(int int_pin, int int_number) : _rtc_int_pin(int_pin), _rtc_int_number(int_number)
{
  init();
}
...
// refresh the buffer
void DS1339::readTime(void)
{
// use the Wire lib to connect to tho rtc
// reset the register pointer to zero
	Wire.beginTransmission(DS1339_CTRL_ID);
...
// request the 7 bytes of data    (secs, min, hr, dow, date. mth, yr)
	Wire.requestFrom(DS1339_CTRL_ID, 7);
	for(int i=0; i<7; i++)
	{
...
// point to start of Alarm1 registers
	Wire.beginTransmission(DS1339_CTRL_ID);
	I2C_WRITE((uint8_t)DS1339_ARLM1);
	Wire.endTransmission();
...
// and that the topmost bit of each helps determine if/how the alarm repeats.
	Wire.requestFrom(DS1339_CTRL_ID, 4);
	for(int i=0; i<4; i++)
	{
                // store data in raw bcd format
		if (Wire.available())
...
void DS1339::writeTime(void)
{
	...
	// clear the Oscillator Stop Flag
        setRegister(DS1339_STATUS, getRegister(DS1339_STATUS) & !DS1339_STATUS_OSF);
}

void DS1339::writeTime(unsigned long sse)
{
...
void DS1339::writeAlarm(void)
{
	Wire.beginTransmission(DS1339_CTRL_ID);
	I2C_WRITE((uint8_t)DS1339_ARLM1); // set register pointer

        I2C_WRITE(rtc_bcd[DS1339_SEC] | ((alarm_repeat & B00000001 ) << 7)); // A1M1
        I2C_WRITE(rtc_bcd[DS1339_MIN] | ((alarm_repeat & B00000010 ) << 6)); // A1M2
        I2C_WRITE(rtc_bcd[DS1339_HR] | ((alarm_repeat & B00000100 ) << 5)); // A1M3
        ...
        if(alarm_repeat & B00001000) // DayOfWeek
        {
            ...
            I2C_WRITE(rtc_bcd[DS1339_DOW] | ((alarm_repeat & B00011000 ) << 3)); // A1M4 and DY/DT
        }
        else // date
        {
            // send date as 4th alarm reg byte
            I2C_WRITE(rtc_bcd[DS1339_DATE] | ((alarm_repeat & B00011000 ) << 3)); // A1M4 and DY/DT
        }
...
void DS1339::writeAlarm(unsigned long sse)
{
        epoch_seconds_to_date(sse);
        writeAlarm();
}

void DS1339::setAlarmRepeat(byte repeat)
{
        alarm_repeat = repeat;
}


unsigned char DS1339::getRegister(unsigned char registerNumber)
{
	Wire.beginTransmission(DS1339_CTRL_ID);
	I2C_WRITE(registerNumber);
	Wire.endTransmission();

	Wire.requestFrom(DS1339_CTRL_ID, 1);

	return I2C_READ();
}

void DS1339::setRegister(unsigned char registerNumber, unsigned char value)
{
	Wire.beginTransmission(DS1339_CTRL_ID);
...
unsigned char DS1339::time_is_set()
{
  // Return TRUE if Oscillator Stop Flag is clear (osc. not stopped since last time setting), FALSE otherwise
  byte asdf = ((getRegister(DS1339_STATUS) & DS1339_STATUS_OSF) == 0);
  return asdf;
}
unsigned char DS1339::alarm_is_set()
{
  // Return TRUE if the alarm interrupt flag is enabled.
  byte asdf = (getRegister(DS1339_SP) & DS1339_SP_A1IE);
  return asdf;
}

void DS1339::enable_interrupt()
{
   clear_interrupt();
   setRegister(DS1339_SP, getRegister(DS1339_SP) | DS1339_SP_INTCN | DS1339_SP_A1IE); // map alarm interrupt to INT1 and enable interrupt
}

void DS1339::disable_interrupt()
{
   setRegister(DS1339_SP, getRegister(DS1339_SP) & !DS1339_SP_A1IE);
}

void DS1339::clear_interrupt()
{
   setRegister(DS1339_STATUS, getRegister(DS1339_STATUS) & !DS1339_STATUS_A1F);
}

unsigned char DS1339::getSeconds()
{
    return bcd2bin(rtc_bcd[DS1339_SEC]);
}

unsigned char DS1339::getMinutes()
{
    return bcd2bin(rtc_bcd[DS1339_MIN]);
}
unsigned char DS1339::getHours()
{
    return bcd2bin(rtc_bcd[DS1339_HR]);
}
unsigned char DS1339::getDays()
{
    return bcd2bin(rtc_bcd[DS1339_DATE]);
}
unsigned char DS1339::getDayOfWeek()
{
    return bcd2bin(rtc_bcd[DS1339_DOW]);
}
unsigned char DS1339::getMonths()
{
    return bcd2bin(rtc_bcd[DS1339_MTH]);
}
unsigned int DS1339::getYears()
{
    return 2000 + bcd2bin(rtc_bcd[DS1339_YR]);
}


unsigned long DS1339::date_to_epoch_seconds(unsigned int year, byte month, byte day, byte hour, byte minute, byte second)
{
...
// NB: The multiplication-by-constants below is intentionally left expanded for readability; GCC is smart and will optimize them to single constants during compilation.
  ...
  sse += 946684800; // correct for difference between DS1339 epoch and UNIX epoch
  return sse;
}


unsigned long DS1339::date_to_epoch_seconds()
{
     unsigned long asdf = date_to_epoch_seconds(int(bcd2bin(rtc_bcd[DS1339_YR])), bcd2bin(rtc_bcd[DS1339_MTH]), bcd2bin(rtc_bcd[DS1339_DATE]), bcd2bin(rtc_bcd[DS1339_HR]), bcd2bin(rtc_bcd[DS1339_MIN]), bcd2bin(rtc_bcd[DS1339_SEC]));
     return asdf;
}



void DS1339::epoch_seconds_to_date(unsigned long seconds_left)
{
   ...
   // NOTE: The earliest date that can be represented by the DS1339 is 1/1/2000 (946684800 in Unix epoch seconds).
   ...
   {
	   seconds_left -= 946684800; // correct for difference between DS1339 and UNIX epochs.

	   seconds_left_2 = seconds_left / 60; // seconds_left_2 = "whole_minutes"
	   rtc_bcd[DS1339_SEC] = bin2bcd(seconds_left - (60 * seconds_left_2));                 // leftover seconds

	   seconds_left = seconds_left_2 / 60; // seconds_left = "whole_hours"
	   rtc_bcd[DS1339_MIN] = bin2bcd(seconds_left_2 - (60 * seconds_left));            // leftover minutes

	   seconds_left_2 = seconds_left / 24; //seconds_left_2 = "whole_days"
	   rtc_bcd[DS1339_HR] = bin2bcd(seconds_left - (24 * seconds_left_2));         // leftover hours
	   ...
	   rtc_bcd[DS1339_DATE] = bin2bcd( days_since_first_of_year - days_to_month + 1);

	   rtc_bcd[DS1339_DOW] = bin2bcd((seconds_left_2  + 4) % 7);


	   //rtc_bcd[DS1339_SEC] = bin2bcd(second);
	   //rtc_bcd[DS1339_MIN] = bin2bcd(minute);
	   //rtc_bcd[DS1339_HR] = bin2bcd(hour);
	   //rtc_bcd[DS1339_DATE] = bin2bcd(day);
	   //rtc_bcd[DS1339_DOW] = bin2bcd(day_of_week);
	   rtc_bcd[DS1339_MTH] = bin2bcd(month);
	   rtc_bcd[DS1339_YR] = bin2bcd(seconds_left);
   }
	else
	{
	...
	// 'Best' way to handle this is to zero out the returned date. 
	   ...
	   rtc_bcd[DS1339_SEC] = 0; //0x00 binary = 0x00 BCD
	   rtc_bcd[DS1339_MIN] = 0;
	   rtc_bcd[DS1339_HR] = 0;
	   rtc_bcd[DS1339_DATE] = 0;
	   rtc_bcd[DS1339_DOW] = 0;
	   rtc_bcd[DS1339_MTH] = 0;
	   rtc_bcd[DS1339_YR] = 0;
	}
...
void DS1339::snooze(unsigned long secondsToSnooze)
{ 
  ...
  setAlarmRepeat(EVERY_MONTH); // There is no DS1339 setting for 'alarm once' - once in a month is the most restrictive it gets.
...
void DS1339::custom_snooze(unsigned long secondsToSnooze)
{ 
...
void DS1339::setSeconds(unsigned char v)
{
    rtc_bcd[DS1339_SEC] = bin2bcd(v);

}
void DS1339::setMinutes(unsigned char v)
{
    rtc_bcd[DS1339_MIN] = bin2bcd(v);

}
void DS1339::setHours(unsigned char v)
{
    rtc_bcd[DS1339_HR] = bin2bcd(v);

}
void DS1339::setDays(unsigned char v)
{
    rtc_bcd[DS1339_DATE] = bin2bcd(v);

}
void DS1339::setDayOfWeek(unsigned char v)
{
    rtc_bcd[DS1339_DOW] = bin2bcd(v);

}
void DS1339::setMonths(unsigned char v)
{
    rtc_bcd[DS1339_MTH] = bin2bcd(v);

}
void DS1339::setYears(unsigned int v)
{
    if (v>1999)
    {
        v -= 2000;
    }
    rtc_bcd[DS1339_YR] = bin2bcd(v);

}

byte DS1339::bcd2bin(byte v)
{
   return (v&0x0F) + ((v>>4)*10);
}

byte DS1339::bin2bcd(byte v)
{
   return ((v / 10)<<4) + (v % 10);
}

void DS1339::stop(void)
{
	setRegister(DS1339_SP, getRegister(DS1339_SP) | DS1339_SP_EOSC);
}

void DS1339::start(void)
{
	setRegister(DS1339_SP, getRegister(DS1339_SP) & !DS1339_SP_EOSC);
}
